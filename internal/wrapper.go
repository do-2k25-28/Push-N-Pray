...
// EnsureNetworkExists creates a Docker network only if it does not already exist. Idempotent.
func (c *Client) EnsureNetworkExists(ctx context.Context, name string) error {
	_, err := c.docker.NetworkInspect(ctx, name, dockerclient.NetworkInspectOptions{})
	if err == nil {
		return nil
	}
	if !strings.Contains(err.Error(), "not found") {
		return fmt.Errorf("dockerwrapper: inspect network %q: %w", name, err)
	}
	if _, err := c.docker.NetworkCreate(ctx, name, dockerclient.NetworkCreateOptions{}); err != nil {
		return fmt.Errorf("dockerwrapper: create network %q: %w", name, err)
	}
	return nil
}
...
func (c *Client) ConnectContainerToNetwork(ctx context.Context, containerName, networkName string) error {
	_, err := c.docker.NetworkConnect(ctx, networkName, dockerclient.NetworkConnectOptions{Container: containerName})
