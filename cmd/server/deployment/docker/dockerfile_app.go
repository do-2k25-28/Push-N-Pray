...
func (app DockerfileApp) RunContainer(ctx context.Context, client Client) error {
	fmt.Printf("Deploying Dockerfile app: %s\n", app.Name())
	if err := client.BuildImage(ctx, app.ImageName(), app.dockerfilePath, app.contextPath); err != nil {
		return fmt.Errorf("failed to deploy app %s: %w", app.Name(), err)
	}
	return app.Run(ctx, client)
}
